...
class Fetch {
  public function __construct() {
    add_action('degg_instagram_fetch', array($this, 'fetch'));
  }

  public function fetch() {
    global $wpdb;
    $instagram = new \Instagram\Instagram;
    $instagram->setClientId( '738dbb952e6a4c44bda0e4ec5952656e' );

    $min_tag_id = get_option('last_instagram_tag_id', 0);
    
    $tag = $instagram->getTag('telluride');
    $media = $tag->getMedia(array('min_tag_id' => $min_tag_id));

    update_option('last_instagram_tag_id', $media->getNextMinId());
    foreach ($media as $m) {
